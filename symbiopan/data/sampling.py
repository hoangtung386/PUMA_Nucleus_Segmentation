...
def compute_sample_weight(
    ...
    rare_tissue_ids = sorted(int(x) for x in np.unique(tissue) if int(x) in RARE_TISSUE_IDS)
...
def compute_all_sample_weights(
    ...
    for i, base_name in enumerate(base_names):
        ...
        if tissue_path.exists():
            ...
            rare_ids = sorted(int(x) for x in np.unique(tissue) if int(x) in RARE_TISSUE_IDS)
            for cls in rare_ids:
                weight += RARE_TISSUE_SAMPLE_BONUS.get(cls, 0.0)
