...
from symbiopan.data.constants import SITE_NAMES
from symbiopan.inference.tiling import autocast_enabled, normalize_tile
from symbiopan.training.checkpoint import extract_state_dict

logger = get_logger(__name__)


def load_site_classifier(checkpoint_path: str | None, device: torch.device, arch: str = "convnext_tiny"):
...
def predict_site_type(
    site_model: torch.nn.Module | tuple, image_rgb: np.ndarray, device: torch.device, image_size: int = 256
) -> int:
